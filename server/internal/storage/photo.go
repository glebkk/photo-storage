...
func (r *PhotoRepository) Create(photo *model.PhotoCreate) (int64, error) {
	...
	if err != nil {
		if pgErr, ok := err.(*pq.Error); ok {
			if pgErr.Code == "23505" {
				return -1, fmt.Errorf("вы уже добавляли фото с таким именем")
			}
		}
...
func (r *PhotoRepository) GetByID(id int64) (*model.Photo, error) {
	row := r.db.QueryRow("SELECT id, user_id, name, file_path FROM photos WHERE id = $1", id)
	photo := &model.Photo{}
	err := row.Scan(&photo.ID, &photo.UserId, &photo.Name, &photo.FilePath)
	return photo, err
}
...
func (r *PhotoRepository) GetAll(userId int64) ([]*model.Photo, error) {
	query := `
        SELECT 
        	p.id, p.user_id, p.file_path, p.name
        FROM 
            photos p
        WHERE 
            p.user_id = $1
	...
	for rows.Next() {
		photo := &model.Photo{}
		if err := rows.Scan(&photo.ID, &photo.UserId, &photo.FilePath, &photo.Name); err != nil {
			return nil, err
		}
